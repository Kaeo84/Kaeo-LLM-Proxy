...
## Round 5 - built-in agents editable/revertible, single default model, upstream types
- Built-ins (Agent/Ask/Plan) moved to `Core/BuiltinAgents.cs` (single source of truth). The Agents tab now lists them first (name read-only), editable via the same Save flow; saves persist only as an override entry when description/prompt differ from defaults; new "Revert to Default" button restores shipped text and drops the override. ToolWindowViewModel applies overrides over built-ins on load, then appends user agents.
- Default model is now exclusive across ALL connections (`UnpinAllExcept` in SettingsWindow replaces per-connection `UnpinOthers`, which was removed). Also fixed a latent bug: models loaded at window open were added before `WireForSave`, so their Enabled/Default edits never persisted; `WireForSave` now wires pre-existing models too. Tool window initial model pick prefers pinned default, then `Defaults.Model`, then first tool-capable.
- Connections carry an `Upstream` type (persisted on `Connection.Upstream`, default "Ollama") with a dropdown (Ollama / OpenAI / Anthropic). New `Core/UpstreamClients.cs`: `IUpstreamClient` (health/models/streaming chat), `UpstreamClientFactory`, and `OpenAiUpstreamClient` / `AnthropicUpstreamClient` stubs that fail with clear NotSupportedExceptions until their API mappings land. `OllamaApiClient` implements the interface; `AgentRuntime`/`ChatEngine`/tool window now program against `IUpstreamClient`. Settings refresh on a stubbed upstream shows the inline refresh error; the tool window skips unreachable/stubbed connections.
- [x] Build clean (0 errors).
- [x] Committed + pushed as Kaeo84.
